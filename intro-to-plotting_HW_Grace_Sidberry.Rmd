...
geom_histogram(binwidth=2) #the type of plot we want to make
```
...
```{r}
ggplot(penguins, aes(x = flipper_length_mm)) + #information about the dataset
  geom_histogram(binwidth=2, color = "white", fill="navy") + #make the figure
...
ggplot(penguins, aes(x = body_mass_g, fill = island)) +
  ...
  geom_histogram(colour = "black")
#Penguins on Biscoe Island must be eating pretty well :)
```
...
ggplot(penguins, aes(x = bill_depth_mm, y = flipper_length_mm, color = species)) +
  geom_point() +
  theme_minimal() +
  scale_color_manual(values = lacroix_palette("PeachPear", type = "discrete")) +
geom_point(size = 2)
```

2B) What happens when you color the data points by body mass (body_mass_g) instead of species?

I tried to run the below code, but it threw me an error, saying that I attempted to supply a continuous value to a discrete scale. Species is categorical, so in this dataset there are three discrete values that it could be. But body mass is continuous because it's a number ranging on a scale. So I can't sort the data points by a continuous value like body mass or bill depth, but I can sort it by discrete values like sex, island, and of course species. 
